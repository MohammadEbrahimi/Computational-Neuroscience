p='C:\Users\Sadegh\Documents\VLMReborn\L347\Data\ConcatDays\';
load(strcat(p,'cellData_ROI.mat'));
load(strcat(p,'SessionLength.mat'));
cellData=cellData_ROINorm;
cellData_Events=zeros(size(cellData),'single');
RWB=3;
RWA=7;
for s=1:length(SessionLength)
   ...
   for cnum=1:size(cellData,1)
       ...
       cellData_Events(cnum,sum(SessionLength(1:s-1))+1:sum(SessionLength(1:s)))=z;

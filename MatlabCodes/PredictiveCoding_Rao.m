...
for i=1:ni
    ...
    for ixf=1:3
        for iyf=1:3
            ...
            %P=norm(segmented_dataset(:,:,ixf,iyf,i));
            %segmented_dataset(:,:,ixf,iyf,i)=segmented_dataset(:,:,ixf,iyf,i) .* RFFilter/P;
        end
    end
end
    ...
    for i=1:trainSize
        ...
        for iter=1:max_iter_r
            ...
            for ixf=1:3
                for iyf=1:3
                    ...
                    r(FilterInd)=r(FilterInd)+(((K1/var1)*U'*(I-(U*r(FilterInd))))+((K1/vartd)*(rtd(FilterInd)-r(FilterInd)))-((K1*alpha)*r(FilterInd))) ;
                    
                    Error1(ixf,iyf,iter)=(norm(I-U*r(FilterInd))^2 / norm(I)^2);
            ...
            rh=rh+(((K1/var1)*Uh'*(r-(Uh*rh)))-(K1*alphah*rh));
            
            Error2(iter)=(norm(r-Uh*rh)^2 / norm(r)^2) ;
        ...
        for ixf=1:3
            for iyf=1:3
                ...
                U=U+((K2/var1)*(((I-(U*r(FilterInd)))*r(FilterInd)')-(K2*lambda*U)));
                
                ErrorT1(ixf,iyf,((br-1)*trainSize)+i)=(norm(I-U*r(FilterInd))^2 / norm(I)^2);
                
            end
        end
        
        Uh=Uh+((K2/var1)*(((r-(Uh*rh))*rh')-(K2*lambda*Uh)));
        ...
        ErrorT2(((br-1)*trainSize)+i)=(norm(r-Uh*rh)^2 / norm(r)^2);
...
FNum=[5,20,24,33,35,43,52,55,58,61];%Old
FNum=[1,9,14,15,19,20,22,23,43,44,51,61,62]
...
% save('E:\Reports2\2018_07_17_PredictiveCoding_RaoModel\Models\Model2_NL','max_iter_r','batchRun','trainSize',...
...
rh=rh/norm(rh);
 ...
 for ixf=1:3
        for iyf=1:3
            ...
            TestError1(ixf,iyf,1)=(norm(I-U*r(FilterInd))^2) / (norm(I)^2);        
        end    
end
...
for iter=1:max_iter_r
    ...
    TestError2(iter)=(norm(r-Uh*rh)^2)/(norm(r)^2);

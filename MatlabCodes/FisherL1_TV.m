% savePath='C:\Users\Sadegh\Documents\VLMReborn\Reports\2016_17_10_FisherTV_TB_ManualMap\DuringStim\HitCR_alldata\L347_L347_all_active_tr_allcells.mat';
% path='C:\Users\Sadegh\Documents\VLMReborn\L347\Data\allDays';
% mode=1;
% train=1;
% TSin=100;
function FisherL1_TV(path,savePath,mode,train,TSin)
   ...
   for indtd=1:length(tdVec)
    ...
    for si=1:length(shuff0)
        ...
        if(max(Speed(SE0(k,1):(SE0(k,1)+55)))<=SpeedTh && max(ActiveAnimal(SE0(k,1):SE0(k,2)))==1 ...
            ...
            stimL=stimL-1;
            if ((SE0(k,1)+j+i) <= SE0(k,2)+1  && stimL>=max(td,0))
    ...
    for si=1:length(shuff1)
        ...
        if(max(Speed(SE1(k,1):(SE1(k,1)+55)))<=SpeedTh && max(ActiveAnimal(SE1(k,1):SE1(k,2)))==1 ...
            ...
            stimL=stimL-1;
            if ((SE1(k,1)+j+i) <= SE1(k,2)+1 && stimL>=max(0,td))
...
%        ResampleMask(1:TSin)=1;
   ...
   shuffInd = [shuffInd0(1:c),(shuffInd1(1:c)+shuffInd0(c))];
    ...
    if train==1
        ...
        [B,intercept,ErrCurve,ErrVar,Lambda]=elasticnetcv(DataSet(:,RM==1),group(RM==1),shuffInd(RM==1),5,10,1,0.9);
        
        % [B,intercept,ErrCurve,ErrVar,Lambda]=glmnoreg(DataSet(:,RM==1),group(RM==1),shuffInd(RM==1),5,10,1);
        
        ErrorCurve{area}{indtd}=ErrCurve;
        ...
        PLSRot{area}{indtd}=PLSRotation;
        
        
    else
        
        B=BMAT(CortexArea==area,indtd,area);
        intercept=InterceptMAT(area,indtd);
        PLSRotation=PLSRot{area}{indtd};
    end
